...
/// Intent routing. Before this existed, any question that was not about a phrase
/// threw `noUsableExpression` — so "what is this paragraph arguing" always failed.
final class ExtractionOutcomeTests: XCTestCase {
    private let lines = [
        "They both signed up to it.",
        "What they wrote is not what shipped.",
    ]

    func testVocabularyIntentYieldsAnExpression() throws {
        let raw = """
            {"intent":"vocabulary","expressions":[
              {"text":"what they wrote","type":"syntax","chinese":"\u{4ed6}\u{4eec}\u{5199}\u{7684}",
               "sentence_position":1}]}
            """
        guard case .vocabulary(let items) = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("expected vocabulary")
        }
        XCTAssertEqual(items.count, 1)
        XCTAssertEqual(items[0].sentencePosition, 1)
    }

    func testComprehensionIntentYieldsAKeptAnswer() throws {
        let raw = """
            {"intent":"comprehension","question":"\u{8fd9}\u{6bb5}\u{5728}\u{8bb2}\u{4ec0}\u{4e48}",
             "answer":"\u{5728}\u{8bf4}\u{5199}\u{51fa}\u{6765}\u{7684}\u{548c}\u{6700}\u{7ec8}\u{505a}\u{51fa}\u{6765}\u{7684}\u{4e0d}\u{4e00}\u{6837}\u{3002}","sentence_position":1}
            """
        guard case .answer(let note) = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("expected an answer")
        }
        XCTAssertEqual(note.question, "\u{8fd9}\u{6bb5}\u{5728}\u{8bb2}\u{4ec0}\u{4e48}")
        XCTAssertFalse(note.answer.isEmpty)
        XCTAssertEqual(note.sentencePosition, 1)
    }

    func testVocabularyClaimWithNothingUsableFallsBackToTheAnswer() throws {
        // The case that used to throw: a real answer thrown away because no phrase
        // could be anchored alongside it.
        let raw = """
            {"intent":"vocabulary","expressions":[],
             "answer":"\u{8fd9}\u{91cc}\u{6ca1}\u{6709}\u{7279}\u{522b}\u{7684}\u{8bcd}\u{ff0c}\u{4f46}\u{8bed}\u{5e8f}\u{503c}\u{5f97}\u{6ce8}\u{610f}\u{3002}"}
            """
        guard case .answer(let note) = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("expected the answer to survive")
        }
        XCTAssertFalse(note.answer.isEmpty)
    }

    func testUnknownIntentIsTreatedAsComprehension() throws {
        let raw = """
            {"intent":"grammar-explanation","answer":"\u{88ab}\u{52a8}\u{8bed}\u{6001}\u{5f3a}\u{8c03}\u{7ed3}\u{679c}\u{3002}"}
            """
        guard case .answer = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("an unrecognised label must not discard the answer")
        }
    }

    func testMissingIntentDefaultsToComprehension() throws {
        let raw = "{\"answer\":\"\u{5c31}\u{662f}\u{5b57}\u{9762}\u{610f}\u{601d}\u{3002}\"}"
        guard case .answer = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("expected an answer")
        }
    }

    func testNoAnswerAndNoExpressionStillFails() {
        // Nothing to show: the only case that should surface an error.
        XCTAssertThrowsError(
            try ExtractionResponse.outcome("{\"intent\":\"comprehension\"}", candidates: lines)
        ) { XCTAssertEqual($0 as? ExtractionParseError, .noUsableExpression) }
    }

    func testOutOfRangePositionIsDroppedRatherThanKept() throws {
        let raw = """
            {"intent":"comprehension","answer":"\u{7b54}\u{6848}","sentence_position":99}
            """
        guard case .answer(let note) = try ExtractionResponse.outcome(raw, candidates: lines) else {
            return XCTFail("expected an answer")
        }
        XCTAssertNil(note.sentencePosition, "99 is not a line; anchoring to it would be a lie")
    }

    func testNonJSONIsStillReportedAsSuch() {
        XCTAssertThrowsError(try ExtractionResponse.outcome("sorry, no", candidates: lines)) {
            XCTAssertEqual($0 as? ExtractionParseError, .notJSON)
        }
    }
}
